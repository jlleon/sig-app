...
export class PanelChartsComponent implements OnChanges {
  ...
  @Input() posicionPasivas: any[];
  @Input() variacionPasivas: any[];
  ...
  ngOnChanges() {
    if (!this.posicionPasivas || !this.variacionPasivas) {
      return;
    }
  ...
  calcularDatosInstituciones() {
    let datos = this.posicionPasivas;
  ...
  calcularDatosVariaciones() {
    let datos = this.variacionPasivas.filter((d): any => d.tipo === this.tipoVariacion);
    if (this.tipoInstitucion !== 'Todas') {
      datos = datos.filter((d): any => d.tipo_institucion === this.tipoInstitucion);
    }
